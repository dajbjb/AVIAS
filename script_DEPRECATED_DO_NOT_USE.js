...
const SyncManager = {
    ...
    testConnection: function () {
        ...
        if (typeof db === 'undefined' || !db) {
            toast.remove();
            alert("❌ Database not initialized. Check internet or Firebase Setup.");
            return;
        }

        db.collection('kingdom_logs').add({
            type: 'connection_test',
            timestamp: new Date()
        }).then(() => {
            toast.remove();
            this.updateStatus(true);
            alert("✅ SERVER CONNECTED! Test message received.");
        }).catch(e => {
            toast.remove();
            this.updateStatus(false, e.message);
            alert("❌ CONNECTION FAILED: " + e.message + "\n\nPossibilities:\n1. Internet is down.\n2. Firebase Rules not updated.\n3. Wrong Project ID.");
        });
    },
    ...
    listenToChats: function () {
        db.collection(this.CHAT_COLLECTION)
            ...
            .onSnapshot((snapshot) => {
                ...
                if (!this.isConnected) console.warn("Sync Error: " + msg);
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    /* --- CRITICAL: RESTORE MISSING LISTENERS --- */
    document.addEventListener('DOMContentLoaded', () => {
        // 1. Chat Send Button (Robust)
        const chatSend = document.getElementById('chat-send-btn');
        const chatInput = document.getElementById('chat-input');
        if (chatSend && chatInput) {
            const sendAction = () => {
                console.log("Chat Send Action Triggered");
                const txt = chatInput.value.trim();
                if (!txt) return;

                if (typeof sendChatContent === 'function') {
                    sendChatContent(txt, 'text', false);
                    chatInput.value = '';
                } else {
                    alert("Error: Chat function missing. Please reload.");
                }
            };
            chatSend.onclick = sendAction;
        ...
        // 2. Chat Camera Button
        const chatCam = document.getElementById('chat-camera-btn');
        if (chatCam) {
            chatCam.onclick = () => {
                const createTab = document.querySelector('[data-tab="create"]');
                if (createTab) createTab.click();
                try { activeCameraMode = 'chat'; } catch (e) { window.activeCameraMode = 'chat'; }
            };
        }

        // 3. Main Capture Button - REWIRED
        const captureBtn = document.getElementById('capture-btn');
        if (captureBtn) {
            captureBtn.onclick = () => {
                const vid = document.querySelector('video'); // Robust query
                const canvas = document.getElementById('camera-canvas');
                const img = document.getElementById('captured-image');
                const ui = document.getElementById('camera-interface');
                const ed = document.getElementById('story-editor');

                if (vid && canvas && img && ui && ed) {
                    ...
                    // Trigger AI Modal if desired, or just show editor
                    const aiModal = document.getElementById('ai-choice-modal');
                    if (aiModal) aiModal.style.display = 'flex';
                } else {
                    console.error("Capture failed: Missing elements", { vid, canvas, img, ui, ed });
                }
            };
        }
    }); // Start Sync safely after DOM load
...
function renderChat() {
    ...
    function sendChatContent(content, type, viewOnce = false) {
        ...
        const snd = document.getElementById('chat-sent-sound');
        if (snd) {
            snd.volume = 0.5;
            snd.currentTime = 0;
            snd.play().catch(e => { });
    ...
    window.startCamera = async function () {
        ...
        if (kingdomCameraStream && kingdomCameraStream.active) {
            if (!vidEl.srcObject) {
                vidEl.srcObject = kingdomCameraStream;
